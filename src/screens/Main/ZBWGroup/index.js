...
const ZBWGroup = () => {
  ...
  const getCurrentMessages = async () => {
    ...
    setUserId(user_id)
    await firestore().collection("chat").orderBy("createdAt", "desc").limit(50).onSnapshot(querySnapshot => {
      const messages = []
      querySnapshot.forEach((res) => {
        const { _id, user, text, createdAt, image, file, video, fileName } = res.data();
        messages.push({ key: res._id, _id, user, text, createdAt, image, file, video, fileName });
      })
  ...
  const renderChatFooter = useCallback(() => {
    ...
    if (videoPath) {
      return (
        <View style={{ flexDirection: 'row' }}>
          <View style={{ width: 100, height: 100, alignItems: 'center', justifyContent: 'center', borderWidth: 1 }}>
  ...
  const renderMessageVideo = (props) => {
    ...
    return (
      <TouchableOpacity
       ...
       style={{height: 350,
        width: 250,backgroundColor:'#000',marginTop:40,alignItems:'center',justifyContent:'center'  }}>
          <Image style={{height:30,width:30}} source={require('../../../assets/Icon/play.png')}/>
      </TouchableOpacity>
    );
  };
  return (
    <View style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
      ...
      <GiftedChat
        ...
        alwaysShowSend={
          text ? true : false || isAttachFile ? true && setText(' ') : false || isAttachImage ? true && setText(' ') : false || isAttachVideo ? true && setText(' ') : false
        }
        renderActions={(props) => (
          <Actions
            {...props}
